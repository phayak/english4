...
class HistoryClassController extends Controller
{
    public function __construct()
    {
        $this->middleware('auth');
    }
    public function historyclass(Request $request)
    {
        $user = Auth::User();
        if($user->status != 0) { return redirect('/'); }
        // $packageNow = Order::where('user_id',$user->id)->select('pay_type','package_id','price_total','status','date_start','date_end','created_at')->get();
        $orders = DB::table('order')
            ->join('package', 'package.id', '=', 'order.package_id')
            ->where('order.user_id','=',$user->id)
            ->orderBy('order.id','desc')
            ->select('order.created_at','order.pay_type','package.name','order.price_total','order.date_start','order.date_end','order.status')
            ->get();
        ...
        return view('user.profile.history',compact('user','orders','packageNow','userBookingCount','userTrialClass','intWorkDay','userBookingofmonth','package_exp','userMakeup','userTrial','packageCount','intHoliday','perDay'));
    }
}
